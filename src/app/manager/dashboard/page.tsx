  ...
  const handleGenerateReportClick = () => {
    ...
    if (teamMembers.length === 0) {
      toast({
        ...
        description:
          "You have no employees assigned to you to generate a report for.",
        variant: "destructive",
      });
      return;
    }
    ...
    receiptsToExport.forEach((receipt) => {
      const vendor =
        receipt.items.find((i) => i.label.toLowerCase() === "vendor")?.value ||
        "N/A";
      const date =
        receipt.items.find((i) => i.label.toLowerCase() === "date")?.value ||
        "N/A";
      const total =
        receipt.items.find((i) => i.label.toLowerCase().includes("total"))
          ?.value || "N/A";
    ...
    link.setAttribute(
      "download",
      `team_activity_report_${new Date().toISOString().split("T")[0]}.csv`
    );
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Oversee expenses, review flagged receipts, and manage your team.
          </p>
        </div>
        <Button
      ...
      <Card className="shadow-md">
        <CardHeader>
          ...
          <CardDescription>
            Overview of expense submissions across all employees.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <TeamActivityTable />
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start gap-3">
            <div>
              ...
              <CardDescription>
                These receipts were flagged by AI and require manual approval.
              </CardDescription>
            </div>
